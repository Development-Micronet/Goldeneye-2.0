...
export function usePredictions() {
    ...
    const hasProcessing = predictions.some(
        ...
        const interval = setInterval(() => {
            setPredictions((prev) =>
                prev.map((item) => {
                    ...
                    const baseline =
                        SERVICE_BY_NAME.get(item.serviceName)?.baselineMs ?? 25_000;

                    const elapsed =
                        Date.now() - (item.processingStartedAt ?? item.startedAt);
    ...
    const run = useCallback(
        async (layer: RasterLayer, services: DetectionType[]) => {
                                ...
                                prev.map((item) =>
                                    item.id === job.id && item.status === "uploading"
                                        ? {
                                            ...
                                            progress: Math.max(
                                                item.progress,
                                                Math.round(fraction * UPLOAD_CEILING)
                                            ),
                                        }
                                        : item
                                )
            ...
            if (failed && settled.length > 1) {
                toast.warning(
                    `${settled.length - failed} of ${settled.length} predictions completed`
                );
            }
        },
