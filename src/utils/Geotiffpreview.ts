...
const readGeoreference = (image: GeoTIFFImage, width: number, height: number): Georeference => {
  const pixelScale = (image.getFileDirectory() as Record<string, any>)?.ModelPixelScale as
    number[] | undefined;
...
const readCaptureDate = (image: GeoTIFFImage): number | undefined => {
  ...
  const iso = raw.replace(/^(\d{4}):(\d{2}):(\d{2})/, (_match, y, m, d) => `${y}-${m}-${d}`);
...
const bandStretch = (
  ...
  return span > 0 ? { min: low, span } : { min: sample[0], span: Math.abs(sample[0]) || 1 };
};
...
export type RasterGeoreference = Omit<RasterPreview, "imageUrl" | "previewWidth" | "previewHeight">;
 ...
 * and tens of seconds.
 */
export async function readGeoTIFFMetadata(file: File): Promise<RasterGeoreference> {
  let tiff: GeoTIFF.GeoTIFF;

  try {
    tiff = await GeoTIFF.fromArrayBuffer(await file.arrayBuffer());
  } catch {
    throw new Error(`${file.name} is not a readable TIFF. Export it as a GeoTIFF and try again.`);
  }
