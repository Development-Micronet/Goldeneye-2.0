...
import { useRasterStore, type DetectionType } from "../store/useRasterStore";
import {
  formatClock,
  formatDuration,
  isActive,
...
import { AoiDrawIcon } from "../../data/components/sidebar/icons/AoiDrawIcon";
...
export const AnalyticsMenu: React.FC = () => {
  ...
  const selectedRasterId = selectedRaster?.id;

  /* a queue picked for one layer should not carry over to the next */
  useEffect(() => {
    setSelectedServices([]);
  }, [selectedRasterId]);
  ...
  const latestByService = useMemo(() => {
    ...
    predictions.forEach((prediction) => {
      if (prediction.layerId !== selectedRasterId) return;
  ...
  /* a rejected run must not surface as an unhandled promise rejection */
  const runServices = async (services: DetectionType[]) => {
    if (!selectedRaster) {
    ...
    try {
      ...
      setSelectedServices((prev) => prev.filter((item) => !services.includes(item)));
    }
  };
    ...
    <div className="font-inter h-full space-y-4 overflow-y-auto p-3">
      ...
      <section className="border-primary/25 rounded-xl border bg-white p-3 shadow-[0_1px_10px_-6px_rgba(44,102,113,0.5)]">
        <LayerSelect
          layers={rasters}
          /*
           * The panel falls back to the last raster when fitRasterId is
           * stale, so passing fitRasterId straight through made the
           * trigger read "Select a layer" while the fields below showed
           * real data. Mirror whatever the panel actually resolved to.
           */
          selectedId={selectedRasterId ?? null}
          onSelect={(id) => {
            setFitRasterId(id);
            ...
            if (layer) {
              toast.info(`${layer.name} selected`);
            }
          }}
        ...
        <div className="bg-primary-100 mt-3 rounded-lg p-2">
          <p className="font-mona text-primary text-[10px] font-semibold tracking-[0.08em] uppercase">
            {/* `aoi` is the source GeoTIFF extent; `extent` is the
                reprojected map extent. Labelled to match the field read. */}
            Source extent
          </p>
      ...
      <section>
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-2">
          {SERVICES.map((service) => (
            <ServiceCard
              ...
              onRunOne={() => void runServices([service.servicename])}
            />
          ))}
        </div>
        ...
        <button
          type="button"
          onClick={() => void runServices(selectedServices)}
        ...
        {results.length === 0 ? (
          ...
          results.map((operation) => (
            <ResultCard
              ...
              onShow={() => showOperation(selectedRasterId, operation.id)}
              onZoom={() => {
                showOperation(selectedRasterId, operation.id);
                requestZoom(selectedRasterId);
              }}
              onRename={(fileName) => renameOperation(selectedRasterId, operation.id, fileName)}
              onDelete={() => {
                removeOperation(selectedRasterId, operation.id);
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="font-mona text-text-secondary block truncate text-[10px] tabular-nums">
                        ...
                        {STAGE_LABEL[prediction.status] ?? prediction.status}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        void run(layer, [prediction.serviceName]).catch((error) => {
                          console.error("[analytics] rerun failed", error);
                          toast.error("Could not start that run again");
                        });
                      }}
