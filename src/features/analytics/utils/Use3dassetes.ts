...
export function featureKey(feature: MapGeoJSONFeature, bbox: Box) {
    const id =
        feature.id ??
        (feature.properties?.osm_id as string | number | undefined) ??
        "anon";

    const round = (value: number) => value.toFixed(5);

    return `${id}:${round(bbox.west)},${round(bbox.south)},${round(
        bbox.east,
    )},${round(bbox.north)}`;
}
...
export function createBuildingGeometry(
    ...
    const minHeight = Math.min(
        getMinHeight(properties),
        height - MIN_EXTRUSION,
    );
...
export function hasVectorLayer(
    map: MapLibreMap,
    sourceId: string,
    sourceLayer: string,
) {
    const source = map.getSource(sourceId) as
        | (VectorTileSource & { vectorLayerIds?: string[] })
        | undefined;

    if (!source || source.type !== "vector") return false;
    return (
        !source.vectorLayerIds ||
        source.vectorLayerIds.includes(sourceLayer)
    );
}
