...
export const OrbitographyMenu: React.FC = () => {
  ...
  const selectedAOIId = useSelectedAOIStore(
    (state) => state.selectedAOIId
  );

  const setSelectedAOI = useSelectedAOIStore(
    (state) => state.setSelectedAOI
  );
  ...
  const [selectedSatellites, setSelectedSatellites] =
    useState<string[]>(SATELLITE_OPTIONS);

  const [showSatelliteDropdown, setShowSatelliteDropdown] =
    useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !satelliteDropdownRef.current.contains(target)
      ) {
  ...
  useEffect(() => {
    if (
      selectedAOIId &&
      layers.some((layer) => layer.id === selectedAOIId)
    ) {
  ...
  const clearAllOrbitLayers = () => {
    ...
    const orbitLayers = state.layers.filter((layer) =>
      layer.label.startsWith("Orbit:")
    );
  ...
  const handleClearOrbit = (pass: any) => {
    ...
    const existingOrbitLayers = state.layers.filter((layer) =>
      layer.label.startsWith(prefix)
    );
  ...
  const getOrbitCoordinates = (
    pass: any
  ): [number, number][] | null => {
    const posList = pass.rawOrbit?.track?.posList;

    if (typeof posList !== "string") {
      const keys = pass.rawOrbit
        ? Object.keys(pass.rawOrbit).join(", ")
        : "null";

      console.warn(
        `Locate failed! No track coordinates. Orbit keys: ${keys}`
      );
    ...
    if (coords.length === 0) {
      console.warn(
        "Locate failed! Coordinate parsing returned empty list."
      );
  ...
  const handleLocateOrbit = (pass: any) => {
    ...
    try {
      const swathFeature = createSwath(
        coords,
        angle
      );
  ...
  const handlePassCheckbox = (pass: any) => {
    const isCurrentlyVisible =
      visiblePassIds.includes(pass.id);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (
        ...
        selectedLayer.geojson.geometry
      ) {
        payload.shape =
          selectedLayer.geojson.geometry;
      }

      const response = await fetchOrbits(
        payload,
      ...
      if (typeof data === "string") {
        try {
          ...
          setDebugMsg(
            `Decryption failed: ${decErr?.message || decErr
            }`
          );
            ...
            if (
              ...
              satItem.orbits.forEach(
                (
                  ...
                  const startStr =
                    orbit.period?.start ||
                    orbit.period?.startDate ||
                    orbit.startTime ||
                    "";

                  const endStr =
                    orbit.period?.end ||
                    orbit.period?.endDate ||
                    orbit.endTime ||
                    "";
    ...
    <div className="flex h-full flex-col overflow-hidden bg-white">
      ...
      <div className="flex flex-col gap-4 border-b border-gray-100 p-4 pb-5">
        ...
        <div className="flex flex-col gap-1.5 font-sans">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none rounded-md border border-gray-200 bg-white px-3 py-2.5 pr-10 text-sm text-gray-800 outline-none transition hover:border-gray-300 focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600"
            >
        ...
        <div
          ...
          ref={containerRef}
        >
          <label className="font-sans text-xs font-semibold text-gray-700">
            Date Range
          </label>
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              ...
              className={`flex flex-1 cursor-pointer select-none items-center justify-between rounded-md border bg-white px-3 py-2 text-sm text-gray-800 ${activePicker === "start"
                ? "border-gray-300"
                : "border-gray-200"
                }`}
            >
            ...
            <div
              ...
              className={`flex flex-1 cursor-pointer select-none items-center justify-between rounded-md border bg-white px-3 py-2 text-sm text-gray-800 ${activePicker === "end"
                ? "border-gray-300"
                : "border-gray-200"
                }`}
            >
        ...
        >
          ...
          <div
            onClick={() =>
              setShowSatelliteDropdown(
                (prev) => !prev
              )
            }
            className="flex w-full cursor-pointer select-none items-center justify-between rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 hover:border-gray-300"
          >
            <span className="truncate text-xs">
              {selectedSatellites.length ===
                SATELLITE_OPTIONS.length
                ? "All 5 satellites"
                : selectedSatellites.length ===
                  0
                  ? "Select Satellites"
          ...
          {showSatelliteDropdown && (
            <div className="absolute left-0 top-[105%] z-50 flex w-full flex-col gap-2 rounded-md border border-gray-200 bg-white p-3 shadow-lg">

              {SATELLITE_OPTIONS.map(
                (satellite) => {
                  const checked =
                    selectedSatellites.includes(
                      satellite
                  ...
                  return (
                    <label
                      key={satellite}
                      className="flex cursor-pointer select-none items-center gap-2.5 text-xs font-semibold text-gray-700"
                    >
                      <input
                        type="checkbox"
                        checked={checked}
        ...
        <div className="mt-1 flex flex-col gap-2 font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="min-w-[45px] text-[10px] font-semibold text-gray-400">
              Max 50°
            </span>
          </div>
        </div>
          ...
          <>
            ...
            <div className="flex flex-col gap-1.5 border-b border-gray-100 bg-gray-50/50 px-4 py-3 font-sans">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  <RefreshCw
                    className={`h-3.5 w-3.5 ${isSearching
              ...
              {debugMsg && (
                <div className="break-all rounded border border-red-100 bg-red-50 p-1.5 text-[10px] font-mono text-red-600">
                  {debugMsg}
                </div>
              )}
            </div>
                ...
                {displayedResults.map(
                  (pass) => {
                    ...
                    return (
                      <div
                        key={pass.id}
                        className={`flex cursor-pointer items-center justify-between rounded-lg border p-2.5 transition ${isChecked
                      ...
                      >
                        ...
                        <button
                          ...
                          className={`rounded-full border p-1 shadow-sm transition ${isChecked
